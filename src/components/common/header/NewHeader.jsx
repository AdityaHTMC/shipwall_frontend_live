...
const NewHeader = () => {
  ...
  useEffect(() => {
    console.log("itemMatrics", itemMatrics)
  }, [itemMatrics])
            ...
            <div className='col-md-8 d-none d-sm-block'>
              <div className='desktopNav'>
                <ul className='menueLinkList d-inline-block'>
                      ...
                      <li key={index}>
                        <div className="customMegaDrop">
                          <div className="dropdown">
                            <Link to={`/product-list/${item?.itmsGrpNam}/${item?.itmsGrpCod}`} onClick={() => toggleDropdown(index, item)} className={`${item.count > 0 ? 'dropdown-toggle dropdown-item2' : ''}`}>
                              {/* <img src={item?.image} alt="" /> */}
                              {item?.itmsGrpNam}
                            </Link>

                            {item?.count > 0 && (
                              <ul className="dropdown-menu dropdown-menu1" style={{ display: showDropdown[index] ? 'block' : 'none' }}>
                                {itemMatrics.map((sub, i) => (
                                  <li className={sub?.count > 0 && "dropdown dropend"} key={i}>
                                    <Link
                                      className={`dropdown-item ${sub?.count > 0 && "dropdown-toggle dropdown-item2"}`}
                                      to={`/product-list/${item?.itmsGrpNam}/${sub?.pname}`}
                                      onClick={(e) => { toggleMultilevelDropdown(e, 2, sub) }}
                                    >
                                      {sub?.pname}
                                    </Link>
                  ...
                  {/* <li><a href='#.'><img src={electronicsIcon} alt="" /> Electronics</a></li>
                    <li><a href='#.'><img src={solarIcon} alt="" /> Solar</a></li>
                    <li><a href='#.'><img src={globesIcon} alt="" /> Globes</a></li>
                    <li><a href='#.'><img src={clearanceIcon} alt="" /> Clearance</a></li>
                   
                    <li><a href='#.'><img src={brandIcon} alt="" /> Brands</a></li> */}
                </ul>
              </div>
            </div>
            <div className='col-md-2 lastItem d-none d-sm-block'>Hotline: <a href="tel:+61432812126">+61 432 812 126</a></div>
          </aside>
        </article>
      </section>
    </div>
